...
- `innerGroups` の CRUD（id, name）
- 未使用 IG の一覧表示（参照パーツが0件）
...
| 画像パス | 必須（現行どおり `file`） |
| インナーグループ | ドロップダウン（任意） |
| マスク画像 | パス入力（任意、`innerGroup` 選択時のみ有効） |

### 7.3 パーツ編集 — マスク指定

| UI | 内容 |
|----|------|
| マスク指定を有効化 | チェックまたは `masksInnerGroups` セクション表示 |
| 対象 IG | `meta.innerGroups` から複数選択。リストの**並び替え**で 5.3.2 の優先順を表現（v1 で競合解決を入れる場合） |

### 7.4 プレビュー（任意）

- 「マスク指定パーツを仮装着」して `activeMaskGroups` をシミュレート
- v1 ではゲーム側での確認でも可
